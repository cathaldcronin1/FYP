...
Graph.prototype = {
  ...
  removeNode: function(id) {
    if (this.nodes[id] && this.nodes[id].shape) {
...
Graph.Renderer.Raphael = function(element, graph, width, height) {
  ...
  this.panZoom = this.r.panzoom({ initialZoom: 0, initialPosition: { x: this.width / 2, y: this.height / 2}});
  this.panZoom.enable();
...
Graph.Renderer.defaultRenderFunc = function(r, node) {
  ...
  var ellipse = r.ellipse(0, 0, 6, 6).attr({
    fill: node.fill || color,
    stroke: node.stroke || color,
    "stroke-width": 2 || stroke_width
  });
  /* set DOM node ID */
  ellipse.node.id = node.label || node.id;
  shape = r.set().push(ellipse).push(r.text(0, 10, node.label || node.id));
  return shape;
}
...
Graph.Layout.TournamentTree.prototype = {
    ...
    layoutCalcBounds: function() {
        // var minx = Infinity, maxx = -Infinity, miny = Infinity, maxy = -Infinity;
        var minx = 0, maxx = 1920, miny = 0, maxy = 1080;
